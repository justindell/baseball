...
def parse_csv file
  ...
  CSV.open(file, {:headers => true, :header_converters => :downcase}).each do |row|
    type = row['ab'] && row['ab'] == '-' ? :pitcher : :batter
    categories = type == :batter ?
      ['r', 'hr', 'rbi', 'sb', 'avg', 'obp'] : 
      ['w', 'k', 's', 'era']
    players[row['name']] = {'type' => type.to_s, 'pos' => row['pos'], 'team' => row['team'], 'value' => 0, 'yahoo_value' => row['mix12'].gsub(/\$/, '').to_f}
    categories.each{|c| players[row['name']][c] = row[c.downcase].to_f}
    if type == :pitcher
      players[row['name']]['h_per_nine'] = (row['hits'].to_f * 9) / row['ip'].to_f
      players[row['name']]['bb_per_nine'] = (row['walks'].to_f * 9) / row['ip'].to_f
...
players = parse_csv '2014_composite_projections_021814.csv'
['r', 'hr', 'rbi', 'sb', 'avg', 'obp'] .each do |stat|
  batters =  players.select{|p,v| v['type'] == 'batter'}
  calculate batters, stat
end
['w', 'k', 's', 'era', 'h_per_nine', 'bb_per_nine'].each do |stat|
  pitchers =  players.select{|p,v| v['type'] == 'pitcher'}
...
players.each do |name, player|
  @players_table.insert :name => name,
                 ...
                 :yahoo_value => player['yahoo_value']
end
